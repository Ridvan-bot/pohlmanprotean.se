    ...
    <section className="relative flex h-screen items-center justify-center overflow-hidden bg-white dark:bg-gray-950 transition-colors duration-300">
      <div className="mx-auto max-w-3xl flex-1 text-center relative z-10">
            ...
            {words.map((word, wordIndex) => (
              <motion.div key={wordIndex} className="flex space-x-1 md:space-x-2">
                {word.split("").map((letter, index) => (
                  <motion.div
                    key={`${wordIndex}-${index}`}
                    className="flex h-10 w-10 items-center justify-center text-2xl font-bold text-gray-900 dark:text-white sm:h-14 sm:w-14 sm:text-3xl md:h-20 md:w-20 md:text-4xl"
                    variants={{
        ...
        >
          ...
          >
            <span className="inline-flex bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-500 dark:to-indigo-200 bg-clip-text text-transparent">
              Innovating for the future
            </span>
          </motion.div>
          <motion.h2
            className="animate-[gradient_6s_linear_infinite] bg-[linear-gradient(to_right,theme(colors.gray.800),theme(colors.indigo.600),theme(colors.gray.900),theme(colors.indigo.700),theme(colors.gray.800))] dark:bg-[linear-gradient(to_right,theme(colors.gray.200),theme(colors.indigo.200),theme(colors.gray.50),theme(colors.indigo.300),theme(colors.gray.200))] bg-[length:200%_auto] bg-clip-text pb-8 font-nacelle text-3xl font-semibold text-transparent md:text-4xl"
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
